...
public class BpmEngineEx extends BPMEngineImpl {
    ...
    @EJB private EtVenderFacade venderFacade;
...
//        System.out.println(workitem.getOwner().getDisplayIdentifier() + " has a workitem id:" + workitem.getId());
    ...
    public void onCompleteProcess(TcProcess process) {
        ...
        if(EtMemberForm.class.getName().equals(className)){
            ...
            List<VenderVO> list = venderFacade.findByCriteria(criteriaVO);
            if(CollectionUtils.isEmpty(list)){
                EtVender entity = new EtVender();
                entity.setMainId(vo.getMemberId());
                entity.setMandt(vo.getMandt());
                entity.setVenderCode(vo.getApplyVenderCode());
                entity.setCname(vo.getApplyVenderName());
                venderFacade.save(entity, operator, Boolean.FALSE);
            }
...
//            }else if(FormTypeEnum.M_I.getCode().equals(vo.getType())){
            ...
            VenderVO vo = vcFormFacade.findById(form.getId(), Boolean.TRUE);
            //save relations
...
//        Map<Long, MemberVO> miMap = new HashMap<>();
        Map<Long, VenderVO> vcMap = new HashMap<>();
...
//                    miWorkitems.add(workitem);
                ...
                VenderVO vo = vcFormFacade.findById(process.getPrimaryobjectid(), Boolean.TRUE);
