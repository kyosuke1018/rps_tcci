...
import com.tcci.ec.model.ApplyFormVO;
...
public class EcMemberFacade extends AbstractFacade<EcMember> {
    ...
    public List<EcMember> findByDisabled(boolean disabled) {
        Query q = em.createNamedQuery("EcMember.findByDisabled");
...
//        sql.append("  web_id1, web_id2, longitude, latitude, start_at, category, capital, year_income, \n");// EC_COMPANY only
     ...
     * @param simulated
     */
    public void save(EcMember entity, TcUser operator, boolean simulated){
        if( entity!=null ){
            // default while null 
...
//        vo.setNickname(vo.getNickname()==null?vo.getCname():vo.getNickname());// for 系統顯示
        ...
        EcMember member = (vo.getMemberId()!=null)?this.find(vo.getMemberId()):new EcMember();
...
//            }
    ...
    public void batchCheckBlockVender(TcUser admin){
        //系統復用 
        List<EcMember> disabledList = this.findByDisabled(Boolean.TRUE);
        if (CollectionUtils.isNotEmpty(disabledList)) {
            for(EcMember entity : disabledList){
        ...
        if (CollectionUtils.isNotEmpty(allList)) {
            ...
            for(EtVender vender : allList){
                ...
                if (CollectionUtils.isEmpty(result)) {
                    ...
                    EcMember entity = this.find(memberId);
...
//        sql.append(" ,v.MANDT, v.LIFNR_UI \n");
        ...
        sql.append("INNER JOIN EC_MEMBER m on m.ID=v.MAIN_ID \n");
...
//        sql.append("AND (S.SPERQ!='99' or S.SPERQ is null ) \n");
    ...
    public void sendPwdMail(EcMember member, String pwd){
