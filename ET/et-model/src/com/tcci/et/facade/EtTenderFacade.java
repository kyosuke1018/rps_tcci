...
import com.tcci.ec.model.criteria.TenderCriteriaVO;
...
//        sql.append("LEFT OUTER JOIN KB_RICH_CONTENT TH ON S.DATATYPE='")
        ...
        sql.append("LEFT OUTER JOIN EC_OPTION EO ON EO.TYPE='area' AND EO.ID=S.AREA_ID \n");
        sql.append("LEFT OUTER JOIN EC_OPTION EO2 ON EO2.TYPE='tenderCategory' AND EO2.ID=S.CATEGORY_ID \n");
...
//        statusList.add(TenderStatusEnum.END.getCode());
        ...
        if (CollectionUtils.isNotEmpty(allTender)) {
            ...
            for(TenderVO vo : allTender){
                ...
                //依案件類別 查通知對象
                List<MemberVO> memberList = ecMemberFacade.findNoticeMemberByCategoryId(categoryId);
