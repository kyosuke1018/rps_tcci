...
public class ImageFacade {
    ...
    public String getFullPath(String imgSrc, String imgType, Long storeId){
        ...
        com.tcci.ec.enums.ImageSizeEnum sizeEnum = com.tcci.ec.enums.ImageSizeEnum.getFromCode(imgType);
        
        if( srcEnum==null ){
            logger.error("getFullFileName srcEnum = null, imgSrc = "+imgSrc);
            return null;
        }
        sizeEnum = (sizeEnum==null)?com.tcci.ec.enums.ImageSizeEnum.ORIGINAL:sizeEnum;
    ...
    public String getFullFileNameSafe(String imgSrc, String imgType, Long storeId, String fileName){
        com.tcci.ec.enums.ImageSizeEnum sizeEnum = com.tcci.ec.enums.ImageSizeEnum.getFromCode(imgType);
        if( com.tcci.ec.enums.ImageSizeEnum.ORIGINAL == sizeEnum ){
            return getFullFileName(imgSrc, imgType, storeId, fileName);
        }else if( com.tcci.ec.enums.ImageSizeEnum.BIG == sizeEnum ){
            String filename = getFullFileName(imgSrc, imgType, storeId, fileName);
            File f = new File(filename);
            if( f.isFile() && f.exists() && f.canRead() ){
                return filename;
            }else{
                return getFullFileName(imgSrc, com.tcci.ec.enums.ImageSizeEnum.ORIGINAL.getCode(), storeId, fileName);
            }
        }else if( com.tcci.ec.enums.ImageSizeEnum.SMALL == sizeEnum ){
            String filename = getFullFileName(imgSrc, imgType, storeId, fileName);
            File f = new File(filename);
            if( f.isFile() && f.exists() && f.canRead() ){
                return filename;
            }else{
                filename = getFullFileName(imgSrc, com.tcci.ec.enums.ImageSizeEnum.BIG.getCode(), storeId, fileName);
                f = new File(filename);
                if( f.isFile() && f.exists() && f.canRead() ){
                    return filename;
                }else{
