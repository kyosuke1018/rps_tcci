            ...
            for (Object row : list) {
                ...
                for(int i=0; i<columns.length; i++){
                    try{
                        ...
                        logger.error("resultSetToPOJOList columns[i].getClass().getName() = " + columns[i].getClass().getName());
                        logger.error("resultSetToPOJOList fieldSequence[i] = " + fieldSequence[i] + "; columns[i] = " + columns[i]);
                        logger.error("resultSetToPOJOList findColIndex IllegalArgumentException:\n", ex);
                    }
                }
    ...
    public Object typeConvertor(Object inObj, T bean, String name) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
        ...
        if( inObj != null ){
            if( inObj.getClass() == BigDecimal.class ){
                ...
                }else if( type == TcUser.class ){// 使用者ID特殊處理
//                    Long bd = (Long)inObj;
//                    outObject = new TcUser(bd);
                    BigDecimal bd = (BigDecimal)inObj;
                    outObject = new TcUser(bd.longValue());
                }/*else if( type == CmFactoryCategory.class ){// 廠類型ID特殊處理
                    BigDecimal bd = (BigDecimal)inObj;
                    outObject = new CmFactoryCategory(bd);
                }*/
            }else if( inObj.getClass() == Long.class ){// for MSSQL
                Class type = PropertyUtils.getPropertyType(bean, name);
                
                if( type == TcUser.class ){// 使用者ID特殊處理
                    Long bd = (Long)inObj;
                    outObject = new TcUser(bd);
                }
            }
