...
import com.tcci.et.model.TcFactoryVO;
...
public class CmUserfactoryFacade extends AbstractFacade<CmUserfactory> {
    private Logger logger = LoggerFactory.getLogger(CmUserfactoryFacade.class);
    ...
    public List<CmFactory> findUserFactoryPermission(TcUser tcUser) {
        ...
        if (CollectionUtils.isNotEmpty(results)) {
            logger.info("findUserFactoryPermission 公司權限設定 CmFactory:"+results.size());
        }
        //其它角色只能看自己所屬的工廠
        Collection<CmUserfactory> userFactoryList = tcUser.getCmUserfactoryList();
        if (CollectionUtils.isNotEmpty(userFactoryList)) {
            logger.info("findUserFactoryPermission 所屬的工廠 CmUserfactory:"+userFactoryList.size());
        }
        if (CollectionUtils.isNotEmpty(userFactoryList)) {
    ...
    public List<CmUserfactory> findByUser(TcUser tcUser) {
        Query q = em.createQuery("SELECT a FROM CmUserfactory a where a.userId=:userId ORDER BY a.prplanFactoryId");
    ...
    public List<TcFactoryVO> findFactoryVOByCriteria(BaseCriteriaVO criteriaVO){
        Map<String, Object> params = new HashMap<>();
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT  \n");
        sql.append("cc.SAP_CLIENT as mandt, cf.SAP_CLIENT_CODE as sapClientCode, \n");
        sql.append("cf.CODE as factoryCode, cf.NAME as factoryName, \n");
        sql.append("'' as categoryCode, '' as categoryName \n");
        
        sql.append("from CM_FACTORY cf \n");
        sql.append("join CM_COMPANY cc on cc.SAP_CLIENT_CODE = cf.SAP_CLIENT_CODE \n");
        sql.append("WHERE 1=1 \n");
        sql.append("and cc.SAP_CLIENT_CODE in ('tcc_cn') \n");

        if( !com.tcci.fc.util.StringUtils.isBlank(criteriaVO.getType()) ){
            sql.append("AND cc.SAP_CLIENT=#MANDT \n");
            params.put("MANDT", criteriaVO.getType());
        }
        if( !com.tcci.fc.util.StringUtils.isBlank(criteriaVO.getCode()) ){
            sql.append("AND cf.CODE=#CODE \n");
        ...
        if( criteriaVO.getOrderBy()!=null ){
            ...
            sql.append("ORDER BY cf.CODE");
        }
        
        List<TcFactoryVO> list = null;
        if( criteriaVO.getFirstResult()!=null && criteriaVO.getMaxResults()!=null ){
            list = this.selectBySql(TcFactoryVO.class, sql.toString(), params, criteriaVO.getFirstResult(), criteriaVO.getMaxResults());
        }else if( criteriaVO.getMaxResults()!=null ){
            list = this.selectBySql(TcFactoryVO.class, sql.toString(), params, 0, criteriaVO.getMaxResults());
        }else{
            list = this.selectBySql(TcFactoryVO.class, sql.toString(), params);
        }
        return list;
    }
}
