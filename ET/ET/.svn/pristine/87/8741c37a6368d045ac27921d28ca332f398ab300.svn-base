...
//                options.add(new SelectItem(g.getId(), g.getCode()+"-"+g.getName()));
    ...
    public void fetchCategoryOptionList(){
        categoryOptionList = new ArrayList<>();
        categoryOptionList = etOptionFacade.findByTypeOptions("tenderCategory", "C");
...
//            block = true;
            ...
            if (CollectionUtils.isNotEmpty(selectedCategorys)) {
                ...
                for(Long cid : selectedCategorys){
                    EtOption etOption = etOptionFacade.find(cid);
                    if( etOption!=null ){
                        if( exists ){
                            sb.append(",").append(etOption.getCname());
                        }else{
                            sb.append(etOption.getCname());
                        }
                        exists = true;
                    }
//                    return sb.toString();
