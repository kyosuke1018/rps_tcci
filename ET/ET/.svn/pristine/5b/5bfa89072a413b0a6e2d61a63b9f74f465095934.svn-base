...
public class ImageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String domainName = request.getParameter("domain");// domain

        logger.debug("ImageServlet ... \n publicImg = " + publicImg 
                + "\n contentType = " + contentType
                + "\n mappingKey = " + mappingKey
                + "\n showImgKey = " + showImgKey
                + "\n imgType = " + imgType
                + "\n domainName = " + domainName);

        // 開放 Image URL
        if( publicImg==null || publicImg.equals("Y") ){// 預設公開網址
            outputImagePublic(response, domainName, mappingKey, contentType, imgType);
        }else{
            // 有 Session 安全檢核
     ...
     * @param contentType 
     */
    private void outputImagePublic(HttpServletResponse response, String domainName, String fileName
            , String contentType, String imgType){
        // 取得要顯示的圖 (小圖、大圖、原圖)
        fileName = imageFacade.getDisplayImage(imgType, domainName, fileName);
        logger.debug("outputImagePublic fileName = "+fileName);
