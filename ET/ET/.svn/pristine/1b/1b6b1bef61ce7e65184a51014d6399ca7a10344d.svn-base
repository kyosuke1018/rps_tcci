...
public class JsfUtils {
    ...
    public static RequestContext buildSuccessCallback(String name, Object value){
        RequestContext context  = buildSuccessCallback();
    ...
    public static RequestContext buildErrorCallback(String name, Object value){
        RequestContext context  = buildErrorCallback();
        context.addCallbackParam(name, value);
        return context;
    }
     ...
     * @return 
     */
    public static boolean resetTreeTable(String treeTableID) {
        try {
            logger.debug("resetTreeTable treeTableID ="+treeTableID);
            
            FacesContext facesContext = FacesContext.getCurrentInstance();
            TreeTable treeTable = (TreeTable) facesContext.getViewRoot().findComponent(treeTableID);
            if( treeTable!=null ){
                // 移除目前排序效果
                treeTable.setValueExpression("sortBy", null);
                treeTable.setFirst(0);

                treeTable.clearDefaultSorted();
            }else{
                logger.error("no TreeTable ID ="+treeTableID);
            }
            return true;
        } catch (Exception e) {
            logger.error("resetDataTable exception :\n", e);
     ...
     * @return 
     */
    public static int getPageDataTable(String dataTableID) {
        try {
            logger.debug("getPageDataTable dataTableID ="+dataTableID);
            
            final DataTable d = (DataTable) FacesContext.getCurrentInstance().getViewRoot().findComponent(dataTableID);
            return d.getFirst();
        } catch (Exception e) {
            logger.error("getPageDataTable exception :\n", e);
            return 0;
        }
    }
