...
public class SecurityUtils {
     ...
     * @return 
     */
    public static Properties getJndiProperties(String JndiName){
        try{
            if( JndiName!=null ){
                Context ctx = new InitialContext();
                return (Properties)ctx.lookup(JndiName);
            }
        }catch(Exception e){
            LOG.error("getJndiProperties Exception JndiName = "+JndiName+": {}", e.getMessage());
        }
        return null;
    }
}
