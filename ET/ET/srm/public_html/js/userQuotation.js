...
var rfqList = [
    ...
    {id: 15, ebelp:15, txz01:"割嘴 G01-100 1#(捷锐PG01-100-1)", menge:50, meins:"PC", lfdat:"2018-02-28", peinh:1000, netpr:21500, netwr:21500, brtwr:1075, waers: 'RMB'},
    {id: 16, ebelp:16, txz01:"移动式电缆盘 YDX10A/220V 40m", menge:4, meins:"PC", lfdat:"2018-02-28", peinh:1, netpr:65, netwr:65, brtwr:260, waers: 'RMB'},
    {id: 17, ebelp:17, txz01:"预沉室入口膨胀节整修,具体详见附图 图号BT-C19-D110-504标识", menge:1, meins:"LE", lfdat:"2018-03-10", peinh:1, netpr:10160, netwr:10160, brtwr:10160, pstyp:9, waers: 'RMB'}
];
var srvItemsMap = {
    ebelp17: [
...
function renderRfqList(){
    var rfqItemHeads = "<tr><th>項次</th><th>品項名稱</th><th>數量</th><th>單位</th><th>計價數量</th><th>單價</th><th>總價</th><th>幣別</th><th>交貨日</th><th>可提供數量</th></tr>\n";
...
function genRfqItemRow(ekpo, index){
    ...
    html += "<td class='tac'>"+ genTextInput("Netwr"+ekpo.id, 8, ekpo.netwr, false) +"</td>\n"; // netpr
    ...
    html += "<td class='tac'>"+ genTextInput("Eindt"+ekpo.id, 10, ekpo.eindt, false) +"</td>\n"; 
    html += "<td class='tac'>"+ genTextInput("Menge"+ekpo.id, 5, ekpo.menge) +"</td>\n"; 
    html += "</tr>\n";

    if( !isNA(ekpo.pstyp) &&  ekpo.pstyp==='9' ){
        var srv = genSrvItemRow(srvItemsMap, ekpo.ebelp);
...
function genSrvItemRow(ekpo, index){
    var html = "<tr>\n";
    html += "<td copspan='8' >...<td>\n";
    html += "</tr>\n";
    return html;
}
