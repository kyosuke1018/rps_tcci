...
//    private UserSession userSession;
    ...
    @PostConstruct
    private void init() {
        templateId = JsfUtils.getRequestParameter("templateId");
        if (null == templateId) {
            JsfUtils.addErrorMessage("參數templateId是空的!");
        } else {
            try {
            ...
            if (null == template) {
                JsfUtils.addErrorMessage("無法找到template!");
            }
        }
    }
    ...
    public void submit() {
        try {
            ...
            TcProcess process = bpmEngine.createProcess(userSession.getLoginTcUser(), template.getProcessname(), mapRoleUsers, subject, primaryObj);
            JsfUtils.addSuccessMessage("流程建立成功, process id=" + process.getId());
        } catch (Exception ex) {
            JsfUtils.addErrorMessage(ex, "create process exception!");
        }
    }
