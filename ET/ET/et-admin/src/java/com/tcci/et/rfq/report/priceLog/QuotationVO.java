...
public class QuotationVO implements Serializable {
    ...
    private String quotationTitle; // 開標日期 / 第一次 / 第二次 / 最終
    private Date quotationTitleDate;

    private QuotationValues quotationValues;

    public QuotationValues getQuotationValues() {
        return quotationValues;
    }

    public void setQuotationValues(QuotationValues quotationValues) {
        this.quotationValues = quotationValues;
    }

    // 總價 for 詢價幣別、詢價數量 
    public BigDecimal getTotalAmtRfqFx() {
        exRate = exRate == null ? BigDecimal.ONE : exRate;
        return totalAmtRfq != null ? exRate.multiply(totalAmtRfq) : null;
    }

    // 淨總價 for 詢價幣別、詢價數量 
    public BigDecimal getNetAmtRfqFx() {
        exRate = exRate == null ? BigDecimal.ONE : exRate;
        return netAmtRfq != null ? exRate.multiply(netAmtRfq) : null;
    }

    // 報價稅率
    public BigDecimal getQuoteTaxRate() {
        if (TaxTypeEnum.ZERO != TaxTypeEnum.getFromCode(taxType)) {
            return this.taxRate;
        }
    ...
    public String getTimesLabel() {
        if (times == null) {
            ...
            return MessageFormat.format("第 {0} 次", times);
        }
    }
    ...
    public void setStatus(String status) {
        ...
        readonly = true;
        if (status != null) {
            if (QuoteStatusEnum.TEMP == QuoteStatusEnum.getFromCode(status)) {
                readonly = false;
            }
        }
    }

    public QuoteStatusEnum getStatusEnum() {
        return QuoteStatusEnum.getFromCode(status);
    }

    public TaxTypeEnum getTaxTypeEnum() {
        return TaxTypeEnum.getFromCode(taxType);
    }
    ...
    public Date getQuotationTitleDate() {
        return quotationTitleDate;
    }

    public void setQuotationTitleDate(Date quotationTitleDate) {
        this.quotationTitleDate = quotationTitleDate;
    }
