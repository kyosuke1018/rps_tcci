...
//        return "SELECT PASSWORD FROM TC_USER WHERE ACCOUNT=?";
        return "SELECT PASSWORD FROM ET_MEMBER WHERE LOGIN_ACCOUNT=?";
    }
...
//        return "SELECT CODE from TC_GROUP G, TC_USERGROUP UG, TC_USER U, ET_MEMBER M WHERE G.ID=UG.GROUP_ID AND UG.USER_ID=U.ID AND U.LOGIN_ACCOUNT = M.LOGIN_ACCOUNT AND U.LOGIN_ACCOUNT=?";
    }
