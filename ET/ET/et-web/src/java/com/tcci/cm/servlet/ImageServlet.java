...
public class ImageServlet extends HttpServlet {
    ...
    private void outputImagePublic(HttpServletResponse response, String storeId, String fileName
        ...
        Long storeIdL = null;
        try{
            storeIdL = storeId!=null?Long.parseLong(storeId):null;
        }catch(Exception e){
            // 共用圖片沒有 storeId
            logger.warn("outputImagePublic imgSrc="+imgSrc+", storeId = "+storeId);
        }
        // 取得要顯示的圖 (小圖、大圖、原圖)
        String fullFilename = imageFacade.getFullFileNameSafe(imgSrc, imgType, storeIdL, fileName);
