...
public class BpmTemplateEdit {
    ...
    public void handleFileUpload(FileUploadEvent event) {
        ...
        try {
            ...
            activityList = ExcelUtils.importList(workbook.getSheet("ACTIVITY"), ActivityVO.class, 1, 1000);
            routeList = ExcelUtils.importList(workbook.getSheet("ROUTE"), RouteVO.class, 1, 1000);
            boolean activityCorrect = verifyActivity();
            boolean routeCorrect = verifyRoute();
            if (activityCorrect && routeCorrect) {
                JsfUtils.addSuccessMessage("資料已讀取!");
            } else {
                JsfUtils.addErrorMessage("資料有誤!");
            }
        } catch (Exception ex) {
            JsfUtils.addErrorMessage(ex, "handleFileUpload exception!");
        }
    }
    ...
    public void save() {
        try {
            ...
            JsfUtils.addSuccessMessage("資料已儲存!");
        } catch (Exception ex) {
            JsfUtils.addErrorMessage(ex, "save exception!");
        }
    }
