...
import com.tcci.fc.facade.bpm.ReassignVO;
...
public class BpmTodoList {
    ...
    public void query() {
        ...
        try {
            ...
            JsfUtils.addErrorMessage(ex, "query exception!");
        }
    }
    ...
    public void sign(TcWorkitem workitem) {
        try {
            ...
            bpmEngine.completeWorkitem(workitem, ballot, route, comments, workitem.getOwner(), allowAgent);
            JsfUtils.addSuccessMessage("簽核成功!");
            query();
        } catch (Exception ex) {
            JsfUtils.addErrorMessage(ex, "簽核失敗!");
        }
    }

    public void reassign(TcWorkitem workitem) {
        try {
            TcUser _newowner = null;
            String _commnets = comments;
            if (null == newAccounts) { // 自動改派 or PASS
                ReassignVO vo = bpmEngine.autoReassign(workitem);
                if (vo != null) {
                    _newowner = vo.getNewOwner();
                    _commnets = vo.getComments();
                }
            } else {
                _newowner = findUser(newAccounts);
            }
            bpmEngine.reassign(workitem, _newowner, _commnets, workitem.getOwner(), allowAgent);
            JsfUtils.addSuccessMessage("改派成功!");
            query();
        } catch (Exception ex) {
            JsfUtils.addErrorMessage(ex, "改派失敗!");
        }
    }
    ...
    public void addApprovers(TcWorkitem workitem) {
        try {
            ...
            JsfUtils.addSuccessMessage("加簽成功!");
            query();
        } catch (Exception ex) {
            JsfUtils.addErrorMessage(ex, "加簽失敗!");
        }
    }
    ...
    public void addActivitiesBefore(TcWorkitem workitem) {
        try {
            ...
            JsfUtils.addSuccessMessage("先會成功!");
            query();
        } catch (Exception ex) {
            JsfUtils.addErrorMessage(ex, "先會失敗!");
        }
    }
    ...
    public void addActivitiesAfter(TcWorkitem workitem) {
        try {
            ...
            JsfUtils.addSuccessMessage("後會成功!");
            query();
        } catch (Exception ex) {
            JsfUtils.addErrorMessage(ex, "後會失敗!");
        }
    }
    ...
    public void notify(TcWorkitem workitem) {
        try {
            ...
            JsfUtils.addSuccessMessage("已發送通知!");
        } catch (Exception ex) {
            JsfUtils.addErrorMessage(ex, "通知失敗!");
        }
    }
