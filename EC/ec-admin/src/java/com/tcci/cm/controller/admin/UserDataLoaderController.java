...
import com.tcci.cm.facade.admin.CsEmployeeFacade;
...
public class UserDataLoaderController extends SessionAwareController implements Serializable {
    ...
    public static final long FUNC_OPTION = 15;
    ...
    @EJB
    private CmFactoryFacade cmFactoryFacade;
    @EJB
    private CmActivityLogFacade cmActivityLogFacade;
    @EJB
    private CsEmployeeFacade csEmployeeFacade;
    ...
    public void resetExport() {
        ...
        JsfUtils.resetDataTable("dataExportForm:dtResult");
    }
        ...
        for (AttachmentVO attachmentVO : list) {
            ...
            try {
                ...
                for (UserLoaderVO vo : importList) {
                    ...
                    vo.setEmployeeName(user.getCname());
                    vo.setEmployeeId(user.getEmpId());
                    vo.setEmailAddress(user.getEmail());
    ...
    /**
     * 維護以下資料 1.TcUser 2.UserInfo 3.TcUsergroup 4.UserFactory
     *
     * @param vo
     * @return
     */
    private TcUser handleTCUser(UserLoaderVO vo) {
        ...
        if (!NULL_STR.equalsIgnoreCase(vo.getEmployeeName())) {
            user.setName(vo.getEmployeeName());
        }else {
            //若傳入的員工姓名為NA，則由CS_EMPLOY取得相關資訊(姓名、員工代號、e-mail)
            EmployeeCriteriaVO cVo = new EmployeeCriteriaVO();
            cVo.setKeyword(vo.getLoginAccount());
            List<CsEmployee> csEmpList = csEmployeeFacade.findEmployeeByAdAccount(vo.getLoginAccount());
            if (CollectionUtils.isNotEmpty(csEmpList)) {
                CsEmployee csEmp = csEmpList.get(0);
                user.setCname(csEmp.getName());
                user.setEmpId(csEmp.getCode());
                user.setEmail(csEmp.getEmail());
            }
        }

        userFacade.save(user, this.getLoginUser());// TC_USER DB儲存

        // 群組 ADMINISTRATORS,SWITCH-USER,...
        ...
        cmActivityLogFacade.logActiveForSingleId(ActivityLogEnum.U_USER_IMPORT, viewId,
     ...
     * 設定使用者工廠
     */
    /*private void setFactorycodes(UserLoaderVO vo, TcUser user) {
        List<CmUserfactory> ppUserfactoryList = user.getCmUserfactoryList();
        if(ppUserfactoryList == null){
            TcUser tcUser = userFactoryFacade.findUserByLoginAccount(user.getLoginAccount());
            ppUserfactoryList = tcUser.getCmUserfactoryList();
        }
        List<String> factoryCodeList = new ArrayList<String>();
        for (CmUserfactory cmUserfactory : ppUserfactoryList) {
            factoryCodeList.add(cmUserfactory.getFactoryId().getCode());
        }
        Collections.sort(factoryCodeList);//排序
        
        StringBuilder factorycodes = new StringBuilder();
        int i = 0;
        for (String factoryCode : factoryCodeList) {
            factorycodes.append(factoryCode);
            if(i != ppUserfactoryList.size()-1){
                factorycodes.append(",");
            }
        ...
        if (CollectionUtils.isEmpty(factoryCodeList)) {
            factorycodes.append(NULL_STR);
        }
        vo.setFactorycodes(factorycodes.toString());
    }*/
    //</editor-fold>
    ...
    /**
     * not used replace by dataExporter
     *
     * @return
     */
