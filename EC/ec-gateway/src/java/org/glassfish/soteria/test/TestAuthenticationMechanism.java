...
public class TestAuthenticationMechanism implements HttpAuthenticationMechanism {
    ...
    protected @EJB TccMemberFacade memberFacade;

    @Override
    public AuthenticationStatus validateRequest(HttpServletRequest request, HttpServletResponse response, HttpMessageContext httpMessageContext) throws AuthenticationException {
        logger.debug("gateway TestAuthenticationMechanism validateRequest");
...
//            if( false ){
                logger.debug("==同步 EC1.0 密碼=="+name);
                memberFacade.syncPasswordFromEc10(name);// 同步 EC1.0 密碼
//            }
        ...
        logger.debug("gateway validateRequest Authorization ="+request.getHeader(HttpHeaders.AUTHORIZATION));
        String token = extractToken(httpMessageContext);
        logger.debug("gateway validateRequest token ="+token);
        if (token != null) {
