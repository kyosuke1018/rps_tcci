 ...
 */
@Stateless
public class EcPrdPaymentFacade extends AbstractFacade<EcPrdPayment> {
    ...
    public EcPrdPaymentFacade() {
        super(EcPrdPayment.class);
    }
     ...
     * @param simulated
     */
    public void save(EcPrdPayment entity, EcMember operator, boolean simulated){
        if( entity!=null ){
     ...
     * @return 
     */
    public List<PrdPaymentVO> findByCriteria(BaseCriteriaVO criteriaVO){
        Map<String, Object> params = new HashMap<String, Object>();
        StringBuilder sql = new StringBuilder();
        if( GlobalConstant.SHARE_PAY_METHOD ){
            sql.append("SELECT T.ID PAY_ID, T.TITLE \n");
            sql.append("FROM EC_PAYMENT T \n");
            ...
            sql.append("FROM EC_PRD_PAYMENT S \n");
            sql.append("JOIN EC_PAYMENT T ON T.ID=S.PAY_ID \n");
            sql.append("WHERE 1=1 \n");
        ...
        List<PrdPaymentVO> list = null;
        if( criteriaVO.getFirstResult()!=null && criteriaVO.getMaxResults()!=null ){
            list = this.selectBySql(PrdPaymentVO.class, sql.toString(), params, criteriaVO.getFirstResult(), criteriaVO.getMaxResults());
        }else if( criteriaVO.getMaxResults()!=null ){
            list = this.selectBySql(PrdPaymentVO.class, sql.toString(), params, 0, criteriaVO.getMaxResults());
        }else{
            list = this.selectBySql(PrdPaymentVO.class, sql.toString(), params);
        }
        return list;
    }
    
    public List<PrdPaymentVO> findByPrd(Long storeId, Long prdId){
    ...
    public void batchRemove(Long storeId, Long prdId, List<Long> idList, boolean include) {
        ...
        sql.append("DELETE FROM EC_PRD_PAYMENT \n");
        sql.append("WHERE 1=1 \n");
    ...
    public boolean checkInput(EcPrdPayment entity, EcMember member, Locale locale, List<String> errors) {
        boolean pass = true;
