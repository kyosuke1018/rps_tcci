...
public class EcTccProductFacade extends AbstractFacade<EcTccProduct> {
    ...
    public String findTccByCriteriaSQL(ProductCriteriaVO criteriaVO, Map<String, Object> params){
        ...
        sql.append("     SELECT * FROM TCCSTORE_USER.EC_PRODUCT S1 \n");
        sql.append("     WHERE S1.ACTIVE=1 \n");
        sql.append("     AND EXISTS ( \n");
        // Jimmy:合約採購不需看 EC_PLANT_PRODUCT
        //sql.append("         SELECT * FROM TCCSTORE_USER.EC_PLANT_PRODUCT A WHERE A.ACTIVE=1 AND A.PRODUCT_ID=S1.ID \n");
        sql.append("         SELECT A.PRODUCT_ID FROM TCCSTORE_USER.EC_PLANT_PRODUCT A WHERE A.ACTIVE=1 AND A.PRODUCT_ID=S1.ID \n");
        sql.append("         UNION \n");
        sql.append("         SELECT A.PRODUCT_ID FROM TCCSTORE_USER.EC_CONTRACT_PRODUCT A, TCCSTORE_USER.EC_CONTRACT B WHERE A.CONTRACT_ID=B.ID AND B.ACTIVE=1 AND A.PRODUCT_ID=S1.ID \n");
        sql.append("     ) \n");
        sql.append("     AND S1.CODE=S.CODE \n");
        sql.append(") \n");
        
        if( criteriaVO.getActive()!=null ){// 可否供加入商店商品
    ...
    public List<ProductE10VO> findProductForImport(){
        ...
        sql.append("JOIN ( \n");
        sql.append("     SELECT PRODUCT_ID FROM TCCSTORE_USER.EC_PLANT_PRODUCT WHERE ACTIVE=1 \n");
        sql.append("     UNION \n");
        sql.append("     SELECT A.PRODUCT_ID FROM TCCSTORE_USER.EC_CONTRACT_PRODUCT A, TCCSTORE_USER.EC_CONTRACT B WHERE A.CONTRACT_ID=B.ID AND B.ACTIVE=1 \n");
        sql.append(") PP ON PP.PRODUCT_ID=S.ID \n");
