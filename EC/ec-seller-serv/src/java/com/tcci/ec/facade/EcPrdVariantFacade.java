...
public class EcPrdVariantFacade extends AbstractFacade<EcPrdVariant> {
    ...
    // 自動產生名稱 CNAME
    public void genVariantName(EcPrdVariant entity, SubmitVO formVO, Locale locale){
        StringBuilder sb = new StringBuilder();
        if( formVO!=null ){
            if( formVO.getColorId()!=null && formVO.getColorId()>0 ){
                sb.append(ProductVariantEnum.COLOR.getDisplayName(locale)).append(":").append(formVO.getColorName());
            }
            if( !sb.toString().isEmpty() ){
                sb.append(",");
            }
            if( formVO.getSizeId()!=null && formVO.getSizeId()>0 ){
                sb.append(ProductVariantEnum.SIZE.getDisplayName(locale)).append(":").append(formVO.getSizeName());
            }
        }
