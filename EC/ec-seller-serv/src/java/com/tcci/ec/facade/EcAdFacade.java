...
public class EcAdFacade extends AbstractFacade<EcAd> {
    ...
    public AdVO findById(AdEnum type, Long id, Boolean fulldata, Locale locale, String opLang){
        if( id==null ){
            return null;
        }
        ProductCriteriaVO criteriaVO = new ProductCriteriaVO();
        ...
        List<AdVO> list = findByCriteria(criteriaVO);
        AdVO vo = (list!=null && !list.isEmpty())?list.get(0):null;
        if( fulldata && vo!=null && vo.getStoreId()!=null ){
            List<LongOptionVO> prdList = productFacade.findOnlinePrdOps(id, locale, opLang);
            vo.setPrdList(prdList);
        }
        return vo;
    }
