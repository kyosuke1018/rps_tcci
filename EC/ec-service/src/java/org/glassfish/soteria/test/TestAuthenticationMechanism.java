...
//    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    ...
    public AuthenticationStatus validateRequest(HttpServletRequest request, HttpServletResponse response, HttpMessageContext httpMessageContext) throws AuthenticationException {
        logger.debug("service TestAuthenticationMechanism validateRequest");
        //new register
        if(request.getParameter("guest") != null 
                || request.getParameter("register") != null 
                || request.getParameter("oid") != null){
            return httpMessageContext.doNothing();
        }
        
        if(request.getUserPrincipal()!=null){
            logger.info("validateToken ... UserPrincipal　{}", request.getUserPrincipal().getName());
        }
...
//            System.out.println("validateRequest result.getStatus() ="+result.getStatus());
            ...
            if( result.getStatus() == VALID ){
                ...
                logger.error("validateRequest Exception:"+e);
                return httpMessageContext.responseUnauthorized();
            }
        }
        logger.debug("service validateRequest Authorization ="+request.getHeader(HttpHeaders.AUTHORIZATION));
        String token = extractToken(httpMessageContext);
        logger.debug("service validateRequest token ="+token);
        if (token != null) {
