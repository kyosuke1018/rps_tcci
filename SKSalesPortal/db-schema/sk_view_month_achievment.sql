CREATE or REPLACE view V_CAL_SALES_MONTH_ACHIEVEMENT AS 
SELECT t1.sapid, trunc(t1.INVOICE_TIMESTAMP,'MONTH') as INVOICE_TIMESTAMP, t1.AMOUNT,t1.TAX,t1.COST 
	,t1.PREMIUM_DISCOUNT, t1.PREMIUM_DISCOUNT_TAX 
	,t1.SALES_DISCOUNT,t1.SALES_DISCOUNT_TAX 
	,t1.SALES_RETURN
	,t1.SALES_AMOUNT
	,t2.BUDGET 
	,((t1.SALES_AMOUNT-t1.COST-t1.CONTRACT_COST + t1.SALES_RETURN)/t1.SALES_AMOUNT) as GROSS_PROFIT_RATE
	,(t1.SALES_AMOUNT / t2.BUDGET) as MONTH_ACHIEVEMENT_RATE
	,t1.CONTRACT_AMOUNT
	,t1.CONTRACT_COST 
FROM 
(
SELECT t1.sapid, trunc(t1.invoice_timestamp,'MONTH') invoice_timestamp, SUM(t1.AMOUNT) as AMOUNT,SUM(t1.TAX) as TAX,SUM(t1.COST) as COST
	,SUM(t1.PREMIUM_DISCOUNT) as PREMIUM_DISCOUNT, SUM(t1.PREMIUM_DISCOUNT_TAX) as  PREMIUM_DISCOUNT_TAX 
	,SUM(t1.SALES_DISCOUNT) as SALES_DISCOUNT,SUM(t1.SALES_DISCOUNT_TAX) as SALES_DISCOUNT_TAX 
	,SUM(t1.SALES_RETURN) as SALES_RETURN 
	,SUM(t1.AMOUNT - t1.PREMIUM_DISCOUNT - t1.SALES_DISCOUNT - t1.SALES_RETURN) as SALES_AMOUNT
	,SUM(t1.CONTRACT_AMOUNT) as CONTRACT_AMOUNT
	,SUM(t1.CONTRACT_COST) as CONTRACT_COST
FROM 
(
...
GROUP BY t1.sapid,trunc(t1.invoice_timestamp,'MONTH')
) t1,
(
