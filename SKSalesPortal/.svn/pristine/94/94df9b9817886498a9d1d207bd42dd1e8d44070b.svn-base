...
public class UserFacade extends AbstractFacade<TcUser> {
    ...
    public TcUser findUserByLoginAccount(String loginAccount) {
        return findUserByLoginAccount(loginAccount, Boolean.FALSE);
    }

    public TcUser findUserByLoginAccount(String loginAccount, Boolean disabled) {
        TcUser obj = null;
        ...
        Predicate p = criteriaBuilder.equal(root.get("loginAccount"), loginAccount);
        predicateList.add(p);
        if (disabled != null) {
            Predicate p2 = criteriaBuilder.equal(root.get("disabled").as(Boolean.class), disabled);
...
//            obj = (TcUser) getEntityManager().createQuery(criteriaQuery).getSingleResult();
    ...
    public List<TcUser> findByCriteria(String input) {
        ...
        Predicate p1 = cb.like(root.get("cname").as(String.class), "%" + input + "%");
        Predicate p2 = cb.like(root.get("loginAccount").as(String.class), input + "%");
        Predicate p3 = cb.like(root.get("email").as(String.class), input + "%");
        predicateList.add(cb.or(p1, p2, p3));
        
        Predicate p4 = cb.equal(root.get("disabled").as(Boolean.class), Boolean.FALSE);
