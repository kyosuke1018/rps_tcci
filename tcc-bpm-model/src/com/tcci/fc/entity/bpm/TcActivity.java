...
public class TcActivity implements Serializable, Persistable {
    // OPTIONS (bits)
    private final static int OPTION_REQUIRED = 1;   // off(預設):無簽核人就pass(approve), on:一定要有人簽
    private final static int OPTION_REQUIREANY = 2; // off(預設):所有人都要簽, on:任一人簽
    private final static int OPTION_AUTOPASS = 4;   // off(預設):簽過的人也要再簽, on:簽過的人自動pass
    ...
    public boolean isOptionRequired() {
        return (options & OPTION_REQUIRED) > 0;
    }
    
    public boolean isOptionRequireAny() {
        return (options & OPTION_REQUIREANY) > 0;
    }
    
    public boolean isOptionAutoPass() {
        return (options & OPTION_AUTOPASS) > 0;
    }
