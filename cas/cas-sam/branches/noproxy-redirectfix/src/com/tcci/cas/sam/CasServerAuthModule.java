        ...
        try {
            ...
            if (assertion != null) {
                ...
                if (principal != null) {
                    ...
                    Object attribute = attributes.get("clientName");
                    String clientName = attribute == null ? "" : attribute.toString();
                    //ccil user add suffix
                    if (!clientName.equals("")) {
                        if (!"EMP ID".equals(clientName)) {
                            assertion = new AssertionImpl(principal.getName().concat("@").concat(clientName));
                        }
                    }
                }
            ...
            //<editor-fold defaultstate="collapsed" desc="old statement">            
////</editor-fold>
            setAuthenticationResult(request, assertion, clientSubject, msgInfo);
            return SUCCESS;
        } catch (Exception e) {
            //e.printStackTrace();
            if (e instanceof TicketValidationException) {
                logger.warning(e.getMessage());
                logger.fine("Redirecting because of an invalid ticket.");
                try {
                    response.sendRedirect(this.casServerUrlPrefix + (!this.casServerUrlPrefix.endsWith("/") ? "/" : "") + "login"
                            + "?service=" + constructServiceUrl(request, response) + "&renew=true");
