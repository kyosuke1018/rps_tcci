...
function resize() {
    ...
    $("#rabbit").hide(0);
    $("#mainPic").attr("src", $("#mainPic").attr("src") + "?" + new Date().getTime());
    $("#mainPic").load(function() {
        $("#rabbit").show(0);
        var rb_width = parseInt($("#rabbit").css("width"));
        if (documentWidth < 1004) {
            $("#rabbit").css("left", bg_width - rb_width + "px");
        } else {
            $("#rabbit").css("left", documentWidth / 2 + bg_width / 2 - rb_width + "px");
        }
    });
}
