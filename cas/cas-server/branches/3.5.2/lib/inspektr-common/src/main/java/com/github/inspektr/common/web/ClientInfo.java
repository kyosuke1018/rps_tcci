...
public final class ClientInfo {
    ...
    public ClientInfo(final HttpServletRequest request) {
        this(request.getLocalAddr(), request.getRemoteAddr(), request.getHeader("X-Forwarded-For"));
    }

    public ClientInfo(final HttpServletRequest request, final String alternateLocation) {
        this(request.getLocalAddr(),
                request.getHeader(alternateLocation) != null ? request.getHeader(alternateLocation) : request.getRemoteAddr(),
                request.getHeader("X-Forwarded-For") != null ? request.getHeader("X-Forwarded-For") : request.getHeader(alternateLocation) != null ? request.getHeader(alternateLocation) : request.getRemoteAddr());
    }

    public ClientInfo(final String serverIpAddress, final String clientIpAddress, final String forwardIpAddress) {
        ...
        this.forwardIpAddress = forwardIpAddress == null ? "unknown" : forwardIpAddress;
    }
