...
public class ServiceBase {
    ...
    private void init() {
        ...
        if (loginMember == null && loginAccount != null) {
            //create EcMember for user who login by oauth.
            logger.debug("loginAccount={}", loginAccount);
            if (loginAccount.contains("profile#")) {
                String json = (String) request.getAttribute(UserSession.CONST_CAS_OAUTH_PROFILE);
                logger.debug("json={}", json);
                BaseProfile profile = new Gson().fromJson(json, BaseProfile.class);
                loginMember = ecMemberFacade.createNewMember(loginAccount, profile);
            } else {
