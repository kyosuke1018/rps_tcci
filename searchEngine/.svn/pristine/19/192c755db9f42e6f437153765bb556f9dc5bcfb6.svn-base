...
/**
 * Copy from HttpSolrServer for SSO
 * (not use extends HttpSolrServer, because HttpSolrServer has to manay private attributes)
 * 
 * @author peter.pan
 */
public class SSOHttpSolrServer extends SolrServer {
    private static Logger log = LoggerFactory.getLogger(SSOHttpSolrServer.class);
     ...
     * User-Agent String.
     */
    public static final String AGENT = "Solr[" + SSOHttpSolrServer.class.getName() + "] 1.0";
    /**
     ...
     * <p/>
     * Parameters that are added to every request regardless. This may be a
     * place to add something like an authentication token.
     */
    ...
    private volatile boolean followRedirects = true; // false; // for SSO
     ...
     * <code>http://localhost:8983/solr/</code>" if you are using the standard
     * distribution Solr webapp on your local machine.
     */
    public SSOHttpSolrServer(String baseURL) {
        this(baseURL, null, new BinaryResponseParser());
    }

    public SSOHttpSolrServer(String baseURL, HttpClient client) {
        this(baseURL, client, new BinaryResponseParser());
    }

    public SSOHttpSolrServer(String baseURL, HttpClient client, ResponseParser parser) {
        this.casServerUri = TcSolrConfig.getSsoTicketURL(); // for SSO
        this.ticketGrantingTicket = SSOClient.getTicketGrantingTicket(casServerUri, TcSolrConfig.getRestUser(), TcSolrConfig.getRestPwd()); // for SSO
        ...
        if (client != null) {
            ...
            if( TcSolrConfig.SSO_SUPPORT_POST ){
                // for SSO POST (apache httpcilent 4.3) 
                httpClient = HttpClientBuilder.create()
                        .setMaxConnPerRoute(TcSolrConfig.DEF_MAX_CONNECT_PER_HOST)
                        .setMaxConnTotal(TcSolrConfig.DEF_MAX_TOTAL_CONNECT)
                        .setRedirectStrategy(new LaxRedirectStrategy()) // *** enabled redirect ***
                        .build();
            }else{
     ...
     * @return 
     */
    public String getUriWithServiceTicket(String service) {
        log.debug("getUriWithServiceTicket... casServerUri = "+casServerUri);
        log.debug("getUriWithServiceTicket... ticketGrantingTicket = "+ticketGrantingTicket);
        log.debug("getUriWithServiceTicket... service = "+service);
        
        String serviceTicket;
        if( TcSolrConfig.SSO_SUPPORT_POST ){
            // serviceTicket = SSOClient.getServiceTicket(casServerUri, ticketGrantingTicket, service, null); // null for post
        }else{
            serviceTicket = SSOClient.getServiceTicket(casServerUri, ticketGrantingTicket, service);
        }
        
        String url;
        if (service.contains("?")) {
            url = service + "&ticket=" + serviceTicket;
        } else {
            url = service + "?ticket=" + serviceTicket;
        }
        log.debug("getUriWithServiceTicket... url = "+url);
            ...
            while (tries-- > 0) {
                ...
                try {
                    if (SolrRequest.METHOD.GET == request.getMethod()) {
                        ...
                        log.info("http get url = " +baseUrl + path + ClientUtils.toQueryString(wparams, false));
                        String serviceWithTicket = getUriWithServiceTicket(baseUrl + path + ClientUtils.toQueryString(wparams, false));
                        method = new HttpGet(serviceWithTicket);
                        ...
                        for (String param : this.queryParams) {
                            ...
                            if (value != null) {
                                ...
                                // wparams.remove(param); // for SSO POST
                            }
                        }
                        ...
                        if (streams == null || isMultipart) {
                            ...
                            log.info("http post url + pstr = " + url + ClientUtils.toQueryString(wparams, false));
                            String serviceWithTicket = getUriWithServiceTicket(url + ClientUtils.toQueryString(wparams, false));
                            HttpPost post = new HttpPost(serviceWithTicket);
                            ...
                            log.info("http podt Multipart url + pstr = \n" + url + pstr);
                            String serviceWithTicket = getUriWithServiceTicket(url + pstr);
                            HttpPost post = new HttpPost(serviceWithTicket);
...
//      }
            ...
            try {
                rsp = processor.processResponse(respBody, charset);// SSO extract error
            } catch (Exception e) {
   ...
   *
    ...
    /**
     * Configure whether the client should follow redirects or not. <p> This
     * defaults to false under the assumption that if you are following a
     ...
     * will be compressed. This method is only allowed if the http client is of
     * type DefatulHttpClient.
     */
    ...
    /**
     * Set maximum number of retries to attempt in the event of transient
     * errors. <p> Maximum number of retries to attempt in the event of
     * transient errors. Default: 0 (no) retries. No more than 1 recommended.
     * </p>
     *
    ...
    /**
     * Set the maximum number of connections that can be open to a single host
     * at any given time. If http client was created outside the operation is
     * not allowed.
     */
     ...
     * status code that may have been returned by the remote server or a proxy
     * along the way.
     */
