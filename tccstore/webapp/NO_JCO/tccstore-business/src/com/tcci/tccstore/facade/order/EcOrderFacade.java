...
public class EcOrderFacade {
    ...
    public void cancelExpiredOrder() {
        ...
        String sql = "SELECT e FROM EcOrder e WHERE (e.status=:status1 OR e.status=:status2) AND e.deliveryDate<:deliveryDate";
        Query q= em.createQuery(sql);
        q.setParameter("status1", OrderStatusEnum.OPEN);
        q.setParameter("status2", OrderStatusEnum.APPROVE);
        q.setParameter("deliveryDate", deliveryDate);
        List<EcOrder> orders = q.getResultList();
        for (EcOrder order : orders) {
            logger.debug("cancelExpiredOrder: order id:{}, original status:{}", order.getId(), order.getStatus());
