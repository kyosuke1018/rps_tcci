...
public class ContractSyncFacade {
    ...
    private final static String vkorgMatches = "2000|2500|3300|3400"; // 英德|貴港|重慶|安順 (上線的廠, 針對新增合約)
    ...
    public ContractSyncData sync(boolean fromSap) throws Exception {
        ...
        if (fromSap) {
            ...
            // loadSapContract(syncData, csvFolder);
        } else {
    ...
    */
    ...
    private void compare1(ContractSyncData syncData) {
        for (String pk1 : syncData.getSetPK1()) {
            ...
            // 電商合約已停用或是測試用的合約 -> 不再同步
            if (ecco != null && (!ecco.isActive() || ecco.getCode().matches("20001234|25001234"))) {
                continue;
            }
            if (null == vbak) { // delete -> disable: SAP合約已停用 (ecco不為null且有有效的)
                ...
                if (!nameChanged && !customerChanged) {
                    continue;
                }
                ContractVO vo1 = new ContractVO(pk1, vbak.getZzadd00(), vbak.getKunnr(), true);
                vo1.setEntity(ecco);
        ...
        for (String pk2 : syncData.getSetPK2()) {
            ...
            if (vbap != null && eccp != null) { // update
                if (!effective) {
                    ...
                    syncData.add(vo2);
                } else if (isContProdChanged(eccp, vo2)) {
        ...
        for (ContractVO vo1 : syncData.getResult1()) {
            ...
            try {
                if (SyncStatus.INSERT == status) {
                    ...
                    syncData.add(ecco);
                    logger.error("contract code:{}, id:{}", ecco.getCode(), ecco.getId());
                } else if (SyncStatus.UPDATE == status) { // 僅修改名稱
