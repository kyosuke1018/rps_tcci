...
public class OrderController {
    ...
    public void initReviewOrder(EcOrder order) {
        ...
        if (order.getMessage() == null || OrderStatusEnum.APPROVE == order.getStatus()) {
    ...
    public void cancel(EcOrder ecOrder) {
        ...
        if (StringUtils.isNotEmpty(ecOrder.getSapOrdernum())) {
            try {
                ...
                SapProxyResponseDto resultDto = SapUtil.soinputCancel(sdProxy, ecOrder);
        ...
        */
        try {
            String orderNotice = msgPrefix + " " + msgInput;
            String comment = MSG_DEFAULT.equals(msgInput) ? null : msgInput;
            ejbFacade.cancelByUser(ecOrder, orderNotice, userSession.getTcUser(), comment);
            zstdFacade.soinputCancel(ecOrder);
        } catch (Exception e) {
            logger.error("cancel(), e={}", e);
            JsfUtil.addErrorMessage(e, "cancel exception!");
            ejbFacade.addLog(ecOrder, "CANCEL_FAIL", userSession.getTcUser(), e.getMessage());
        }
    }
    ...
    private void approveOrder(EcOrder ecOrder) {
        ...
        try {
            ...
            sdProxy.dispose();
            */
            ejbFacade.approveByUser(ecOrder, userSession.getTcUser());
            zstdFacade.soinputCreate(ecOrder);
            JsfUtil.addSuccessMessage(rb.getString("order.msg.approveSuccess"));
        } catch (Exception e) {
            logger.error("approve(), e={}", e);
            JsfUtil.addErrorMessage(e, "approveOrder exception!");
