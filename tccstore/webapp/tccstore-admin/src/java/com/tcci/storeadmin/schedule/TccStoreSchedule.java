...
public class TccStoreSchedule {
    ...
    @EJB
    private CustomerSyncFacade customerSyncFacade;
    ...
    public void dailySyncOrderStatus() {
        try {
            ...
            if (scheduleFacade.canExecute("dailySyncOrderStatus", 3)) {
                ...
                orderJobs.tmpOrderStatusSync(200); // 每次200筆
            } else {
    ...
    public void dailyDeliveryPlaceSync() {
        ...
        try {
            ...
            if (scheduleFacade.canExecute("dailyDeliveryPlaceSync", 3)) {
                ...
                notify = syncData.hasAnyResult(); // 有異動再發email
            } else {
    ...
    public void dailyContractSync() {
        ...
        try {
            ...
            if (scheduleFacade.canExecute("dailyContractSync", 3)) {
                // PRODUCTION　always sync from SAP
                // QA/TEST disable this schedule
                // boolean fromSap = "SAP".equals(tccstoreConfig.get("syncFrom"));
                syncData = contractSyncFacade.sync(true);
                contractSyncFacade.save(syncData);
                notify = syncData.hasAnyResult(); // 有異動才發email
            } else {
    ...
    public void dailyCustomerSync() {
        ...
        CustomerSyncData syncData = null;
        String syncError = "";
        try {
            if (scheduleFacade.canExecute("dailyCustomerSync", 3)) {
                execute = true;
                syncData = customerSyncFacade.sync();
                customerSyncFacade.save(syncData);
            } else {
                logger.warn("dailyCustomerSync not execute");
            }
        } catch (Exception ex) {
            logger.error("dailyCustomerSync exception", ex);
            if (ex instanceof EJBException) {
                ex = getRootCause((EJBException) ex);
            }
            syncError = "错误原因:" + ex.getMessage();
        ...
        if (execute) {
            HashMap<String, Object> mailBean = new HashMap<>();
            String subject = "[台泥电商] 客戶(含業務,客戶業務)同步结果";
            mailBean.put(VelocityMail.SUBJECT, subject);
            mailBean.put(VelocityMail.TO, "jimmy.lee@tcci.com.tw");
            mailBean.put("syncData", syncData);
            mailBean.put("syncError", syncError);
            VelocityMail.sendMail(mailBean, "/mail_customerSyncNotify.vm");
        }
    }

    // 批次同步SAP開單/取消結果
    // SAP是每5分鐘處理(0,5,10,...), 這邊晚3分鐘處理(3,8,13,...)
    @Schedule(hour = "*", minute = "3/5", persistent = false)
