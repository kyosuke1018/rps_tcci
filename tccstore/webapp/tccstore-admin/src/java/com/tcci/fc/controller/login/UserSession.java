...
public class UserSession implements Serializable {
    ...
    private List<TcGroup> plantGroups = new ArrayList<>();
    ...
    private void loadUserInfo() {
        ...
        if (null == tcUser) {
            ...
            // 使用者可檢視的廠
            for (TcUsergroup ug : tcUser.getTcUsergroupCollection()) {
                TcGroup tcGroup = ug.getGroupId();
                if (tcGroup.getCode().matches("^[1-9][0-9]00$")) {
                    plantGroups.add(tcGroup);
                }
            }
            Collections.sort(plantGroups, new Comparator<TcGroup>() {
                @Override
                public int compare(TcGroup o1, TcGroup o2) {
    ...
    public List<TcGroup> getPlantGroups() {
        return plantGroups;
    }

    public void setPlantGroups(List<TcGroup> plantGroups) {
