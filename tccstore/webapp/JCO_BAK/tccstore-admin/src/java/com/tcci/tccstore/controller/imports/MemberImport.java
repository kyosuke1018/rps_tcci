...
public class MemberImport extends ImportExcelBase<MemberVO> {
    ...
    protected boolean postInit(MemberVO vo) {
        if (!setPK.add(vo.getLoginAccount())) {
            vo.setMessage(vo.getLoginAccount() + " loginAccount duplicated!");
            return false;
        }
        EcMember ecMember = entityFacade.findByLoginAccount(vo.getLoginAccount());
