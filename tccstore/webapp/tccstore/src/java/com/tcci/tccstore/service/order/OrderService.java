...
public class OrderService extends ServiceBase {
    ...
    */
    ...
    @Path("cancel/{order_id}")
    @Produces(MediaType.TEXT_PLAIN)
    public String cancelOrder(@Context HttpServletRequest request, @PathParam("order_id") Long orderId) throws ServiceException {
        EcOrder order = getAuthOrder(orderId);
        ResourceBundle rb = ResourceBundle.getBundle("/msgOrder", request.getLocale());
        OrderStatusEnum status = order.getStatus();
        logger.debug("cancelOrder:{}, current status:{}", orderId, status);
        // 已出貨不允許取消
        if (OrderStatusEnum.CLOSE == status) {
            String error = rb.getString("order.err.orderClosed");
            logger.error(error);
            ...
            return rb.getString("order.msg.cancelSuccess");
        } else if (OrderStatusEnum.FAIL == status) { // 已失敗，不要再送
            String error = "订单状态失败!";
            logger.error(error);
            throw new ServiceException(error);
        } else {
            String orderNotice = MessageFormat.format(rb.getString("order.msg.orderCancelNotify"),
                            new Object[]{String.valueOf(order.getId()), order.getQuantity(), order.getVehicle()});
            orderFacade.cancelByMember(order, orderNotice);
            zstdFacade.soinputCancel(order);
            return rb.getString("order.msg.cancelSuccess");
        }
    }
...
//    }
    ...
    */
    ...
    @Produces(MediaType.TEXT_PLAIN)
    public String createOrder(@Context HttpServletRequest request, Order order) throws ServiceException {
        logger.debug("createOrder json:{}", toJson(order));
        EcMember loginMember = getAuthMember();
        ResourceBundle rb = ResourceBundle.getBundle("/msgOrder", request.getLocale());
        EcOrder ecOrder = validateAndConvert(order, rb);
        ecOrder.setMemberId(loginMember);
        // c1開的單不能自動核准
        boolean isC1 = "c1".equals(loginMember.getLoginAccount());
        boolean autoOrder = ecOrder.getPlantId().isAutoOrder();
        OrderStatusEnum status = (!isC1 && autoOrder) ? OrderStatusEnum.APPROVE : OrderStatusEnum.OPEN;
        ecOrder.setStatus(status);
        orderFacade.createOrder(ecOrder, rb.getString("order.msg.orderCreateNotify"));
        if (OrderStatusEnum.APPROVE == status) {
            // SAP批次開單
            zstdFacade.soinputCreate(ecOrder);
        }
        logger.debug("createOrder id:{}, success!", ecOrder.getId());
        return rb.getString("order.msg.createSuccess");
    }
...
//    }
    private List<String> validate(Order order, ResourceBundle rb) {
        ...
        if (order.getContract() != null && order.getPosnr() == null) {
            errorList.add(rb.getString("order.err.contractProductPosnrIsRequired"));
        }
    ...
    */
    ...
    private void cancelSapOrder(EcOrder order) {
        try {
            ...
            SapProxyResponseDto resultDto = sdProxy.soinputCancel(params);
    ...
    */
    ...
    private EcOrder validateAndConvert(Order order, ResourceBundle rb) {
        Customer customer = order.getCustomer();
        EcCustomer ecCustomer = (null == customer) ? null : getAuthCustomer(customer.getId());
        Plant plant = order.getPlant();
        EcPlant ecPlant = (null == plant) ? null : ecPlantFacade.find(plant.getId());
        Product product = order.getProduct();
        EcProduct ecProduct = (null == product) ? null : ecProductFacade.find(product.getId());
        DeliveryPlace delivery = order.getDelivery();
        EcDeliveryPlace ecDelivery = (null == delivery) ? null : ecDeliveryPlaceFacade.find(delivery.getId());
        String vkorg = (null == ecPlant) ? null : ecPlant.getVkorg();
        EcDeliveryVkorg devk = (null == delivery) ? null : ecDeliveryVkorgFacade.find(delivery.getId(), vkorg);
        EcSalesarea ecSalesarea = (null == devk) ? null : devk.getEcSalesarea();
        Sales sales = order.getSales();
        EcSales ecSales = (null == sales) ? null : ecSalesFacade.find(sales.getId());
        Contract contract = order.getContract();
        EcContract ecContract = (null == contract) ? null : ecContractFacade.find(contract.getId());
        // 檢查必填欄位
        StringBuilder errors = new StringBuilder();
        validateNotNull(ecCustomer, rb.getString("order.err.customerIsRequired"), errors);
        validateNotNull(ecPlant, rb.getString("order.err.plantIsRequired"), errors);
        validateNotNull(ecProduct, rb.getString("order.err.productIsRequired"), errors);
        validateNotNull(ecDelivery, rb.getString("order.err.deliveryPlanceIsRequired"), errors);
        validateNotNull(ecSalesarea, "无对应的销售区域!", errors);
        validateNotNull(ecSales, rb.getString("order.err.salesIsRequired"), errors);
        if (contract != null) {
            validateNotNull(ecContract, "contract id not exists!", errors);
            validateNotNull(order.getPosnr(), rb.getString("order.err.contractProductPosnrIsRequired"), errors);
        }
        validateNotNull(order.getQuantity(), rb.getString("order.err.quantityIsRequired"), errors);
        validateNotNull(order.getVehicle(), rb.getString("order.err.vehicleIsRequired"), errors);
        validateNotNull(order.getMethod(), rb.getString("order.err.methodIsRequired"), errors);
        String deliveryDate = order.getDeliveryDate();
        if (validateNotNull(deliveryDate, rb.getString("order.err.deliveryDateIsRequired"), errors)) {
            try {
                SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
                Calendar cal = Calendar.getInstance();
                String today = sdf.format(cal.getTime());
                cal.add(Calendar.DATE, 1);
                String tomorrow = sdf.format(cal.getTime());
                if (!deliveryDate.equals(today) && !deliveryDate.equals(tomorrow)) {
                    validateNotNull(null, rb.getString("order.err.deliveryDateNG"), errors);
                }
            } catch (Exception e) {
                validateNotNull(null, rb.getString("order.err.deliveryDateNG"), errors);
            }
        ...
        if (errors.length() > 0) {
            logger.error("validateAndConvert error:{}", errors.toString());
            throw new ServiceException(errors.toString());
        }
        EcOrder ecOrder = new EcOrder();
        ecOrder.setCustomerId(ecCustomer);
        ecOrder.setPlantId(ecPlant);
        ecOrder.setPlantCode(ecPlant.getCode());
        ecOrder.setPlantName(ecPlant.getName());
        ecOrder.setProductId(ecProduct);
        ecOrder.setProductCode(ecProduct.getCode());
        ecOrder.setProductName(ecProduct.getName());
        ecOrder.setDeliveryId(ecDelivery);
        ecOrder.setDeliveryCode(ecDelivery.getCode());
        ecOrder.setDeliveryName(ecDelivery.getName());
        ecOrder.setSalesareaId(ecSalesarea);
        ecOrder.setSalesareaCode(ecSalesarea.getCode());
        ecOrder.setSalesareaName(ecSalesarea.getName());
        ecOrder.setSalesId(ecSales);
        ecOrder.setSalesCode(ecSales.getCode());
        ecOrder.setSalesName(ecSales.getName());
        if (ecContract != null) {
            ecOrder.setContractId(ecContract);
            ecOrder.setContractCode(ecContract.getCode());
            ecOrder.setContractName(ecContract.getName());
    ...
    private boolean validateNotNull(Object field, String nullMessage, StringBuilder errors) {
        if (null == field) {
            if (errors.length() !=0 ) {
                errors.append('\n');
            }
            errors.append(nullMessage);
            return false;
        }
        return true;
    }
