 ...
 */
@Entity
@Table(name = "RPT_DATA_UPLOAD_RECORD")
@NamedQueries({
    @NamedQuery(name = "RptDataUploadRecord.findByYearmonthCompany",
        query = "SELECT r FROM RptDataUploadRecord r"
        + " WHERE r.yearmonth=:yearmonth AND r.companyCode=:companyCode"),
    @NamedQuery(name = "RptDataUploadRecord.findByYearmonth",
        query = "SELECT r FROM RptDataUploadRecord r"
        + " WHERE r.yearmonth=:yearmonth")
})
public class RptDataUploadRecord implements Serializable {
