...
//        System.out.println("uploadContent");
        ...
        Boolean fvitemCreated=false;
        TcFvitem fvitem=null;
        try {
            //
            fvitem = createFvitem(uploadedFileName, filesize, domain, contenttype);
            fvitemCreated=true;
            TcFvitem fvItem = buildFvItem(fvitem, domain, inputstream, uploadedFileName, filesize, contenttype);
            System.out.println("fileUploadFacade fvItem.getId()=" + fvItem.getId());
            ...
            applicationData.setDescription("test");
...
//            System.out.println(e);
    ...
    private TcFvitem buildFvItem(TcFvitem fvItem, TcDomain domain, InputStream inputstream, String uploadedFileName, long filesize, String contenttype) throws Exception {
        //get Full file name
...
//        fvItem.setId(getFvItemId());
        System.out.println("buildFvItem1 fvItem.getId()=" + fvItem.getId());
        String server_fileName = fvItem.getId() + "." + extension;
...
//        }
        ...
        System.out.println("buildFvItem2 fvItem.getId()=" + fvItem.getId());
        String server_folder = tcFvvault.getLocation();

        uploadAttachment(inputstream, server_folder, server_fileName);
        System.out.println("buildFvItem3 fvItem.getId()=" + fvItem.getId());
...
//        fvItem.setContenttype(contenttype);
        fvItem = em.merge(fvItem);
        System.out.println("buildFvItem4 fvItem.getId()=" + fvItem.getId());
        return fvItem;
    }

    private TcFvitem createFvitem(String clientFileName, long fileSize, TcDomain domain, String contentType) throws Exception {
        TcFvitem fvitem = new TcFvitem();
        fvitem.setName(clientFileName);
        fvitem.setFilename("update_later");
        fvitem.setFilesize(fileSize);
        fvitem.setDomain(domain);
        fvitem.setContenttype(contentType);
        Principal principal = sessionContext.getCallerPrincipal();
