...
public class PDFExporter extends Exporter {
        ...
        try {
            ...
            while (st.hasMoreElements()) {
                ...
                if (tableTitle != null && !tableTitle.isEmpty() && !tableId.contains("" + ",")) {
                    ...
                    Paragraph title = new Paragraph(tableTitle, facetFont);
                    document.add(title);
    ...
    protected void tableFacet(FacesContext context, PdfPTable pdfTable, DataTable table, int columnCount, String facetType) {
        ...
        if (component != null) {
            ...
            if (component instanceof HtmlCommandButton) {
                ...
                //cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                addColumnAlignments(null, component, cell);
                    ...
                    for (UIComponent rowComponent : row.getChildren()) {
                        ...
                        if (null != header) {
                            if (header.getChildCount() > 0) {
                                for (UIComponent colComponent : header.getChildren()) {
                                    if (colComponent.isRendered()) {
                                        value = exportValue(FacesContext.getCurrentInstance(), colComponent);
                                    }
                                }
                            } else {
                        ...
                        addColumnAlignments(column, component, cell);
                        pdfTable.addCell(cell);
    ...
    protected void exportRow(DataTable table, PdfPTable pdfTable, int rowIndex) {
        ...
        if (sr != null && sr.isInView()) {
            for (UIComponent summaryComponent : sr.getChildren()) {
                ...
                addColumnAlignments(column, column.getChildren(), cell);
                pdfTable.addCell(cell);
            }
        }
    }
        ...
        for (UIColumn col : table.getColumns()) {
            ...
            if (col.isExportable()) {
                if (col.getSelectionMode() != null) {
                    PdfPCell cell = new PdfPCell(new Paragraph(col.getSelectionMode(), this.cellFont));
                    addColumnAlignments(col, col.getChildren(), cell);
                    pdfTable.addCell(cell);
                    continue;
                }
                addColumnValue(pdfTable, col, col.getChildren(), this.cellFont);
            }
        ...
        for (UIColumn col : table.getColumns()) {
            ...
            if (col.isExportable()) {
                if (col.getHeaderText() != null && columnType.name().equalsIgnoreCase("header")) {
                    ...
                    addColumnValue(pdfTable, col, col.getFacet(columnType.facet()), this.facetFont);
                }
            }
        }
    }
    ...
    protected void addColumnValue(PdfPTable pdfTable, UIColumn column, UIComponent component, Font font) {
    ...
    protected void addColumnValue(PdfPTable pdfTable, UIColumn column, List<UIComponent> components, Font font) {
        StringBuilder builder = new StringBuilder();
        ...
        addColumnAlignments(column, components, cell);
        pdfTable.addCell(cell);
    }
    ...
    protected void addColumnAlignments(UIColumn column, UIComponent component, PdfPCell cell) {
        Integer colAlign = null;
        if (component != null && component instanceof HtmlOutputText) {
            HtmlOutputText output = (HtmlOutputText) component;
            Map<String, String> styleMap = parseStyle(output.getStyle());
            if (styleMap.containsKey("text-align")) {
                if ("left".equalsIgnoreCase(styleMap.get("text-align"))) {
                    colAlign = Element.ALIGN_LEFT;
                } else if ("right".equalsIgnoreCase(styleMap.get("text-align"))) {
        ...
        if (colAlign == null) {
            if (column != null) {
                Map<String, String> styleMap = parseStyle(column.getStyle());
                if (styleMap.containsKey("text-align")) {
                    if ("left".equalsIgnoreCase(styleMap.get("text-align"))) {
                        colAlign = Element.ALIGN_LEFT;
                    } else if ("right".equalsIgnoreCase(styleMap.get("text-align"))) {
    ...
    protected void addColumnAlignments(UIColumn column, List<UIComponent> components, PdfPCell cell) {
        Integer colAlign = null;
        if (components != null) {
            for (UIComponent component : components) {
