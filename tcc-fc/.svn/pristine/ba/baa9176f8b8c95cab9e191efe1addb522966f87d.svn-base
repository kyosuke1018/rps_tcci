...
public class TcProcessTemplateFacade {
    ...
    public void save(TcProcesstemplate processTemplate, List<IActivityVO> activityList, List<IRouteVO> routeList) {
    ...
    private void processActivties(List<IActivityVO> activityList, Collection<TcActivitytemplate> origActs, 
            Collection<TcActivitytemplate> newActs, Collection<TcActivitytemplate> delActs,
            TcProcesstemplate processTemplate) {
        Map<String, IActivityVO> mapIActivityVO = new HashMap<String, IActivityVO>();
        for (IActivityVO avo : activityList) {
            mapIActivityVO.put(avo.getActivityname(), avo);
        }
        for (TcActivitytemplate origAct : origActs) {
            String key = origAct.getActivityname();
            IActivityVO avo = mapIActivityVO.get(key);
            if (null == avo) {
        ...
        for (IActivityVO avo : mapIActivityVO.values()) {
    ...
    private void updateActivity(TcActivitytemplate activity, IActivityVO avo) {
    ...
    private void processRoutes(List<IRouteVO> routeList, Collection<TcActivityroutetemplate> origRoutes,
        ...
        Map<String, IRouteVO> mapIRouteVO = new HashMap<String, IRouteVO>();
        for (IRouteVO rvo : routeList) {
        ...
        for (TcActivityroutetemplate origRoute : origRoutes) {
            ...
            IRouteVO rvo = mapIRouteVO.get(key);
            if (null == rvo) {
        ...
        for (IRouteVO rvo : mapIRouteVO.values()) {
    ...
    private void updateRoute(TcActivityroutetemplate route, IRouteVO rvo, Map<String, TcActivitytemplate> mapActivity) {
