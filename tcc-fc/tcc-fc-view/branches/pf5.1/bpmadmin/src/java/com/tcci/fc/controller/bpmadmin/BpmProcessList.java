...
public class BpmProcessList {
    ...
    public void terminateProcess(TcProcess row) {
        try {
            bpmEngine.terminateProcess(row, terminateReason, userSession.getTcUser());
            JsfUtil.addSuccessMessage("流程(" + row.getId() + ")已停止!");
            query();
        } catch (Exception ex) {
            JsfUtil.addErrorMessage(ex, "terminateProcess exception!");
        }
    }
