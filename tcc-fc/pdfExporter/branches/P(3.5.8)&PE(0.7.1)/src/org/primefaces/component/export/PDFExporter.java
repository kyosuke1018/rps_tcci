...
//    	this.facetFont = FontFactory.getFont(FontFactory.TIMES, encoding, Font.DEFAULTSIZE, Font.BOLD);
        BaseFont bf = null;
        try {
            bf = BaseFont.createFont("MHei-Medium", "UniCNS-UCS2-H", BaseFont.EMBEDDED);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected void addColumnFacets(DataTable table, PdfPTable pdfTable, ColumnType columnType) {
        for (UIColumn col : table.getColumns()) {
            ...
            if (col.isExportable()) {
                ...
                //---End---tcc customized.
                addColumnValue(pdfTable, col.getFacet(columnType.facet()), this.facetFont, colAlign);
            }
        }
    }

    protected void addColumnValue(PdfPTable pdfTable, UIComponent component, Font font, int align) {
